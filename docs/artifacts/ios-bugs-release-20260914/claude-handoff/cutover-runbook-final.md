...
Status: **EXECUTED 2026-09-15 06:03Z to 06:28Z** on Jackson's approval; see `../cutover-20260915/timeline.txt` and `identities.json` for the actual deployment ids, ledger versions and timestamps. Deviations from the plan as written: (1) the live site had moved to main `81f55e56c`, so both builds were rebased there first (hold `e73cb9e25`, candidate `3fc2f6d76`) with byte-identical release content; (2) the full type check revealed a production compile error in `sage/reconcile/route.ts` caused by widening the queue entity union, fixed on both trees before any promotion, and the earlier empty type logs are now known to be out-of-memory false negatives; (3) the bundle was submitted through the same migration mechanism but as a transcription that was hash-verified byte-for-byte locally before submission and again against the stored ledger bytes afterwards; (4) the archived-job reopen migration was applied immediately after the bundle on Jackson's instruction. Originally: prepared and reviewed, not executed. Requires Jackson's explicit backend release approval for the exact scope in the last section. This runbook is the day-of procedure; `cutover-analysis.md` is the underlying analysis and `runbook-review.md` is the independent review whose six required changes are folded in here.
