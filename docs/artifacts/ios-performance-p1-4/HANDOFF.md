...
## Combined core04 fixture corrections

PM's integration `8ef739cd` ran 309 tests: 306 passed, three failed, none skipped. Both original lead-capture crashes were resolved. Six of the seven driver lifecycle tests passed, including current-session actor completion and invalidation/resume; the PM also reported the actual typed visit/media invalidation cases passing. The current worker read `core-tests-04-summary.json`, `lifecycle-reset-04-details.json`, `mention-retarget-04-details.json`, and `core04-relevant-diagnostics/test-stdout.txt` from the integration artifact directory. No worker runtime commands were run.

Two assigned failures were test-fixture defects:

- The reset test reached `SwiftData/ModelContext.swift:324`, with `Container does not have any data stores` (stdout 189–192). Its final readback constructed a new ModelContext after `container.deleteAllData()` had removed the stores. The test now retains the synthetic reset and awaited CancellationError assertion, and stops before reusing the destroyed container. Completion of that await exercises both the callback guard and captured-ID claim-release defer.
- `ProjectNoteMentionEditTests/testLongLivedDataActorRefreshesMainContextDispatchRetarget` let real connectivity transition to connected and start a `projectNote` dispatch immediately before the dependency assertion at line 3193 (stdout 547–555). The mutation correctly preserves an in-progress/actively leased dispatch's dependency; this case assumes it is pending. The fixture now passes an always-offline ConnectivityManager through the existing subclass seam, including the suite's local relaunch fixture. It explicitly asserts that the dispatch is unclaimed and independently reads the persisted new dependency before checking the long-lived actor's refreshed readiness. The production retarget/lease and actor-refresh implementation is unchanged; those queue/controller files also had no P1-4 changes against the initial baseline.

Frontend parse of both edited test files and CRLF-aware whitespace checking exited 0. PM rerun needed: `OPSTests/OutboundLifecycleTests` and `OPSTests/ProjectNoteMentionEditTests` under the existing serial baton. The unrelated core04 queue-policy failure remains P1-2's assignment. No new production behavior or Bible delta in these fixture corrections.
