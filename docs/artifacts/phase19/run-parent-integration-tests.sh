...
cd "$root"
test ! -e "/private/tmp/ops-site-visits-p19/phone-tests-$run.xcresult"
test ! -e "docs/artifacts/phase19/phone-app-tests-$run.log"
xcodebuild test -project OPS.xcodeproj -scheme OPS \
  ...
  -only-testing:OPSTests/CalendarMirrorContentTests \
  -only-testing:OPSTests/CalendarSiteVisitLeadResolverTests \
  -only-testing:OPSTests/SiteVisitTypeSettingsInputTests \
