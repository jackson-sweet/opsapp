...
- **Comment attachment removal (`f5f57917`) has a missing live server prerequisite.** iOS fixes `9eac1406`/`82a01604` and render harness `c3ee720c` are ancestors of main. The app sends `p_attachments` when changing media, but production exposes only the four-argument `update_project_note_mentions(uuid,text,text[],uuid)` (MD5 `9d16ec64ff2d15da9e00fe1fbeb1adc4`), so the changed-media call cannot resolve. The existing staged SQL is `docs/migrations/2026-09-04-03-update-project-note-mentions-attachments.staged.sql`; it is outside the five verified migrations above and still needs current-baseline review, database verification and separately authorized application. Text-only calls retain the four-argument shape. Original photos owner preserved.
