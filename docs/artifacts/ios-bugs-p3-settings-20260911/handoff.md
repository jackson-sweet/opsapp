...
No confirmed source path bypasses the global installer. No new functional keyboard repair is justified yet. The only production-file change is removing `private` from `SiteVisitTypeEditorView` so the test target can host the real editor; layout, focus, styling, copy, persistence and keyboard code are identical.

## Prepared regression

New suite: `OPSTests/Views/SiteVisitTypeSettingsKeyboardTests.swift` (three tests).

- Presents the actual `SiteVisitTypeEditorView` inside a SwiftUI sheet and NavigationStack in `AppHostWindow`; it does not recreate the input controls in a mock editor.
...
- Checks NAME and FIELD LABEL independently, including attachment to a visible window, a fully visible minimum-sized DONE target, an actual rendered accessory attachment, UIKit dismissal, the sheet remaining open and draft text surviving an update in another field.
...
**Execution is outstanding.** Per the root's serial build baton, this worker ran no compiler, build, test, simulator, device, install or release command. `git diff --check` passes. Root should run only `OPSTests/SiteVisitTypeSettingsKeyboardTests` initially, with the simulator software keyboard enabled. A visibility failure must first distinguish a disabled software keyboard/test-host presentation failure from the original production report. Existing `SiteVisitTypeSettingsInputTests` remains unchanged.
