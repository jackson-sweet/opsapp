...
Jackson authorized repair sessions and delegated project management to the parent task `01a0779e-54bf-72a3-976c-81c692990d23`. He separately authorized a local full-phone-database export if useful. The parent alone handles that export; never copy real database contents into git, artifacts, tests, or other services. Workers receive aggregate/schema evidence and use synthetic fixtures. Subsequent explicit approval authorizes the exact reviewed stage-command production migration, data-preserving optimized test installation on Jackson's paired iPhone, and online/offline measurements. The migration is applied as20260907001000 and the build installed; measurement remains in progress. No pushes, unrelated deployments/mutations, or App Store releases are authorized. Atomic local commits and local integration are authorized by the repair request.
...
Profile a known optimized build using the scripted new visit → checklist → ten photos → notes → representative deck → return → save/reopen flow, with online/offline/slow-network/reconnect conditions and long-lived synthetic workloads based on measured aggregate counts. The optimized in-place phone install and measurement are explicitly approved. Unrelated customer-data mutations and App Store release remain separate gates. Do not claim actual device or customer-live performance solely from synthetic tests.
