...
**Implementation status (2026-06-30):** Complete. Added `DeckPatternMeshBuilder`, `OverheadSceneNodes`, and `DeckSceneLayerToggle`; wired `DeckSceneBuilder` to render `surfaceFeatures.patterns` as one textured SceneKit mesh per matched surface and `overhead.structures` as an overhead layer rooted at `layer.overhead`. Existing flat deck surface rendering remains the fallback when no pattern spec resolves. Overhead members reuse geometry for repeated dimensions, and the layer toggle hides only the overhead subtree.

**Verification (2026-06-30):** `swift test --package-path Packages/DeckKit --filter DeckScenePhase6RenderTests` (5 tests, including the Phase 6 SceneKit snapshot attachment), `swift test --package-path Packages/DeckKit` (455 tests), `scripts/verify-ops-decks-style-tokens.sh .`, `git diff --check`, `xcodebuild -project OPS.xcodeproj -scheme OPS -destination 'platform=iOS Simulator,name=iPhone 17,OS=26.5' -derivedDataPath /private/tmp/ops-decks-p6-task10-final-app-dd CODE_SIGNING_ALLOWED=NO test -only-testing:OPSTests/DeckSceneBuilderPhase6RenderTests`, and `xcodebuild -project OPS.xcodeproj -scheme OPSDecks -destination generic/platform=iOS -derivedDataPath /private/tmp/ops-decks-p6-task10-final-app-dd -disableAutomaticPackageResolution CODE_SIGNING_ALLOWED=NO build` all passed. A fresh `OPSDecks` DerivedData build attempted first failed before compile because SwiftPM tried to clone `swift-protobuf` submodules and GitHub DNS was blocked (`Could not resolve host: github.com`); the successful build used the resolved package graph from the passing app test.
...
**Interface:** SwiftUI views taking the standalone `DeckDrawingEditorModel` capability context (C§4). No new public DeckKit types beyond the views; internal toolbar/runner support stays inside DeckKit. The views mutate `DeckDrawingData.surfaceFeatures`, edge-level `stairConfig`, and `.overhead` through the editor model's existing persist path.
...
**Implementation status (2026-06-30):** Complete. Added Phase 6 authoring capabilities (`surfacePatterns`, `stairDetails`, `surfaceFeatures`, `overheadStructures`) to `DeckCapabilities.full` while leaving `.light` unchanged except for the single Pro upsell stub. Added internal `DeckSurfaceEditorToolbarModel` and `DeckSurfaceEditorEngineRunner` so LIGHT mode cannot reach pattern/stair/feature/overhead mutations or sizing engines. Added the four public SwiftUI sheets: `SurfacePatternSheet` with live pattern previews, `StairDetailSheet` with one flight-focused entry and open/closed/mono plus steel/aluminum/PT/cedar stringer choices, `SurfaceFeaturesSheet` for fasteners/fascia/skirting/finish/built-ins/lighting, and `OverheadStructureSheet` as a single CONNECT-style entry. Wired `DeckDrawingEditorView` to present the FULL sheets via `sheet(item:)`; the OPS light surface receives only the passive "Available in OPS Decks Pro" stub.

**Verification (2026-06-30):** Added and passed `DeckDrawingEditorModelTests/testSurfaceSheet_hiddenWhenCapabilityAbsent`, `testEngineNeverInvokedInLight`, and `testFullSurfaceEditorsPersistPhase6Blocks`. Ran `swift test --package-path Packages/DeckKit --filter DeckDrawingEditorModelTests/testSurfaceSheet_hiddenWhenCapabilityAbsent --filter DeckDrawingEditorModelTests/testEngineNeverInvokedInLight --filter DeckDrawingEditorModelTests/testFullSurfaceEditorsPersistPhase6Blocks` (3 tests), `scripts/verify-ops-decks-style-tokens.sh .`, `swift test --package-path Packages/DeckKit` (458 tests), `git diff --check`, and `xcodebuild -project OPS.xcodeproj -scheme OPSDecks -destination generic/platform=iOS -derivedDataPath /private/tmp/ops-decks-p6-task10-final-app-dd -disableAutomaticPackageResolution CODE_SIGNING_ALLOWED=NO build`; all passed. The pre-build process check found no active `xcodebuild`/`swift test` processes. The `OPSDecks` build reused the resolved package graph to avoid network package-resolution drift.
