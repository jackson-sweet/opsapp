...
//  OPS
//
//  Schema version 13.0.0 — Unified activity parents.
//
//  V13 widens the `Activity` model so a timeline activity can be parented to a
...
//
//  `Activity` shipped through V1–V12 inside `OPSSchemaCommon.unchangedModels`,
//  so widening it in place would shift EVERY historical schema's fingerprint by
...
//  `OPSSchemaLegacyActivity.Activity` and backs V1–V12 (`v1ToV12ActivityModel`);
//  the live widened `Activity` backs V13+ (`v13ActivityModel`). Because the two
//  Activity shapes differ by real persistent attributes, V13's checksum diverges
//  from V12 organically — the same "mint a real new-shape model" discipline that
...
//  lightweight migration handles the on-disk V12 store transparently: existing
