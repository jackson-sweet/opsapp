...
#if canImport(UIKit)
import UIKit
public typealias DeckKitPlatformImage = UIImage
#elseif canImport(AppKit)
import AppKit
public typealias DeckKitPlatformImage = NSImage
#endif

@MainActor
public class SketchScanPipeline: ObservableObject {

    public enum ScanStage: String {
        case idle = "Ready"
    ...
    public func process(image: DeckKitPlatformImage, measurementSystem: MeasurementSystem = .imperial) async {
        guard let cgImage = image.deckKitCGImage else {
            error = "Invalid image"
            stage = .failed
            return
        }
...
private extension DeckKitPlatformImage {
    var deckKitCGImage: CGImage? {
        #if canImport(UIKit)
        return cgImage
        #elseif canImport(AppKit)
