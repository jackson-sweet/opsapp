...
## 10. Implementation note — Guided Catalog Setup authors this model (2026-06-10)

The Guided Catalog Setup flow now writes into both halves of this model directly, with **zero schema change** — every primitive below already existed and is already consumed by a live read path. See `docs/superpowers/plans/2026-06-10-guided-catalog-variants-tiers.md`.

**Option-priced tiers on a service/good line (product side).** A line such as "Full Detail — Sedan $180 / SUV $230 / Truck $280" is committed as one `products` row plus one `product_options` row (`kind='select'`, `affects_price=true`, `default_value=<lowest-tier label>`), one `product_option_values` row per tier, and one `product_pricing_modifiers` row (`modifier_kind='add_flat'`) per non-default tier. **Base price = the lowest tier; each modifier amount = tier price − base** (so deltas are ≥ 0 and the picker headline reads as the floor price). The estimate builder consumes this unchanged: `ProductPickerSheet` routes a product with options to `LineItemEditSheet`, `seedDefaultsForOptions()` pre-selects the default tier, and `ProductConfigurationResolver.resolve()` prices `base + Σ fired modifiers`, snapshotting `resolved_unit_price` / `configured_options` / `resolved_options_label`. The estimate line row now renders `resolved_options_label` so the chosen tier is visible. Price-only (no per-tier cost) in v1 — there is no per-option-value cost column. `products.tiered_pricing_json` is **not** used (it does not exist as a column in prod and is never read).

**Variant matrices on an inline-created assembly material (catalog side).** A create-new material with option axes (Color → black/white, optionally × Thickness → the 24-variant vinyl matrix) is committed as one `catalog_items` family + one `catalog_options` row per axis + one `catalog_option_values` row per value + one `catalog_variants` row per combo (cartesian) + the `catalog_variant_option_values` links. Cost + unit are family-level in v1. The package's `product_materials` recipe row is **variant-pinned to the first generated variant** — `RecipeResolver` consumes a variant pin or a `$option` selector, and a nil-selector family pin is silently dropped from the cut list (`chk_product_materials_pin_xor_family` permits the row, but the resolver requires a pin). The full labeled family lives in stock (counted + labeled by `CatalogVariantLabeler`) and is pickable in future packages. The assembly pick-existing picker now labels variants by their option-value combo instead of raw SKU, via the shared `CatalogVariantLabeler`.
