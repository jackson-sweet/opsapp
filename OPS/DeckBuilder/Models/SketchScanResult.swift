...
struct DetectedLineSegment: Identifiable {
    let id: String
    ...
    init(id: String = UUID().uuidString, startPoint: CGPoint, endPoint: CGPoint) {
        self.id = id
...
struct DetectedVertex: Identifiable {
    let id: String
    let position: CGPoint
    var connectedSegmentIds: [String]

    init(id: String = UUID().uuidString, position: CGPoint, connectedSegmentIds: [String] = []) {
        self.id = id
...
struct StairPattern: Identifiable {
    let id: String
    let boundingRect: CGRect
    let internalLineCount: Int
    let nearestSegmentId: String?
...
struct ContourExtractionResult {
    let vertices: [DetectedVertex]
    let segments: [DetectedLineSegment]
    let isClosed: Bool
    let stairPatterns: [StairPattern]
}
...
struct ScaleConflict {
    let segmentId: String
    let annotatedInches: Double
    let scaleDerivedInches: Double
