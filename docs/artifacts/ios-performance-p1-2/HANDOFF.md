...
- Actual CREATE LEAD saves and injected failures for both new and existing clients, unrelated caller WIP, atomic client/extra-contact/draft/outbox rollback, retry deduplication, deliberate contact-field clears, nonblank note updates, phone field protection and stopped-create non-bypass. An actual resumed, already-client-bound blank draft preserves stored client notes and another screen's pending notes; neither its payload nor changedFields includes a note clear. Retry of a locally saved pending client still probes visibility.
