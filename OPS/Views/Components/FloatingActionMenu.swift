...
struct FloatingActionMenu: View {
    ...
    private func orderedMoneyItems(rawItems: [FABMenuItem]) -> [FABMenuItem] {
        ...
        // Merge resolution (2026-05-12): Pipeline-tab P1-2's `isLeadsTab` branch
        // wraps the segment switch (so LEADS always promotes "add-lead"); the
        // inner switch uses Reconstruction's post-Pipeline-split shape — no
        // "PIPELINE" segment case anymore (Pipeline lives in its own LEADS tab),
        // and the default fallback is "new-invoice" (the new BOOKS default
        // segment is INVOICES, not the now-removed PIPELINE).
        if isLeadsTab {
            primaryId = "add-lead"
        } else {
    ...
    var body: some View {
        ZStack {
            ...
            if showCreateMenu || isEditMode {
                ...
                .ignoresSafeArea()
                .transition(.opacity)
                .animation(OPSStyle.Animation.fast, value: showCreateMenu)
                .onTapGesture {
                    ...
                    VStack(alignment: .trailing, spacing: 4) {
                        ...
                        ForEach(section.items, id: \.id) { item in
                            ...
                            editModeItemRow(item: item, isHidden: isHidden)
                                ...
                                .animation(isDragging ? nil : OPSStyle.Animation.fast, value: itemVisualOffset(for: item.id, in: section.items))
                                .gesture(
