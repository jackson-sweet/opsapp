...
extension Font {
    ...
    /// header voice (MOBILE.md §2.1): uppercase, left-aligned, `Colors.text`.
    ...
    public static var screenTitle: Font {
        return Font.custom("CakeMono-Light", size: screenTitleSize, relativeTo: .title)
    }
    ...
    public static var screenTitleLong: Font {
        return Font.custom("CakeMono-Light", size: screenTitleLongSize, relativeTo: .title2)
    }

    /// Nominal point sizes behind `screenTitle` / `screenTitleLong`. Exposed so
    /// layout code can derive the width-compression floor (22/28) instead of
    /// duplicating the values.
    public static let screenTitleSize: CGFloat = 28
    public static let screenTitleLongSize: CGFloat = 22
