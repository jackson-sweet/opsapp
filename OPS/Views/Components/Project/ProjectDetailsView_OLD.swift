...
/// A photo for display in the gallery, with optional caption from note attachments
...
/// OLD: Retained as reference during migration. Not compiled.
/// Pointed pill shape - pill with a triangular point on the left side (like a map callout)
struct ProjectDetailsView_OLD: View {
    ...
    @State private var showingProjectNotes = false
    @State private var showingNoteImagePicker = false
    @State private var noteSelectedImages: [UIImage] = []
    @State private var showingNotePhotoViewer = false
    @State private var notePhotoViewerURLs: [String] = []
    @State private var notePhotoViewerIndex: Int = 0
        ...
        Group {
            if isDeleting {
                ...
                mainView
                    ...
                    .sheet(isPresented: $showingNoteImagePicker) {
                        noteImagePickerContent
                    }
                    .fullScreenCover(isPresented: $showingNotePhotoViewer) {
    ...
    /// Task color hexes for the project's active tasks (used by map pin renderer).
    private var projectTaskColorHexes: [String] {
        project.tasks
            .filter { $0.deletedAt == nil && $0.status == .active }
            .map { $0.effectiveColor }
    }

    /// Pin label for the hero map — street number + street name (first part of address).
    private var pinLabel: String {
        guard let address = project.address, !address.isEmpty else {
            return project.title
        }
        // Take the first component (street number + name) before the first comma
    ...
    /// Map background with 4:3 aspect ratio and segmented-ring project pin.
    /// Uses Mapbox with the same dark OPS style as the main map.
        ...
        ZStack(alignment: .topTrailing) {
            if let coordinate = project.coordinate {
                ProjectLocationMapView(
                    ...
                    taskColorHexes: projectTaskColorHexes,
                    isInteractive: true,
                    zoomLevel: 15.0
                )
                .frame(height: height)
    ...
    private func contentWithGradient(mapHeight: CGFloat) -> some View {
        ZStack(alignment: .top) {
            ...
            VStack(spacing: 0) {
                ...
                // Gradient from 1/3 to 2/3 of map — gradual fade
                LinearGradient(
                    gradient: Gradient(stops: [
                        .init(color: Color.clear, location: 0),
                        .init(color: OPSStyle.Colors.background.opacity(0.15), location: 0.25),
                        .init(color: OPSStyle.Colors.background.opacity(0.35), location: 0.5),
                        .init(color: OPSStyle.Colors.background.opacity(0.65), location: 0.75),
    ...
    private var photosNotesTabContent: some View {
        VStack(alignment: .leading, spacing: 20) {
            ...
            // NOTES section (inline message board)
            inlineNotesSection
    ...
    private var photosTabSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            let displayPhotos = allDisplayPhotos
            ...
            if displayPhotos.isEmpty {
                ...
                LazyVGrid(columns: [
                    ...
                    ForEach(Array(displayPhotos.enumerated()), id: \.element.id) { index, displayPhoto in
                        galleryThumbnailView(displayPhoto: displayPhoto, index: index, allPhotos: displayPhotos)
                            .aspectRatio(1, contentMode: .fit)
    ...
    // MARK: - Inline Notes Section (Message Board)

    private var inlineNotesSection: some View {
        VStack(alignment: .leading, spacing: OPSStyle.Layout.spacing2) {
            ...
            if notesViewModel.isLoading && notesViewModel.notes.isEmpty {
                ...
                // Empty state
                Text("No notes yet — post a note for your team")
                    .font(OPSStyle.Typography.caption)
                    .foregroundColor(OPSStyle.Colors.tertiaryText)
                    .padding(.horizontal)
            } else {
                // Show notes (all of them, scrollable inline)
                    ...
                    ForEach(notesViewModel.notes) { note in
                        InlineNoteRow(
                            ...
                            onPhotoTap: { urls, index in
                                notePhotoViewerURLs = urls
                                notePhotoViewerIndex = index
                                showingNotePhotoViewer = true
                            }
                        )
                    }
                }
            ...
            VStack(spacing: 0) {
                ...
                HStack(spacing: OPSStyle.Layout.spacing1) {
                    ...
                    // Camera button
                    Button(action: { showingNoteImagePicker = true }) {
    ...
    private var photoViewerContent: some View {
        let photos = project.getProjectImages()
        let safeIndex = min(selectedPhotoIndex, max(photos.count - 1, 0))
    ...
    private var notePhotoViewerContent: some View {
        PhotoCommentViewer(
            photos: notePhotoViewerURLs,
            initialIndex: notePhotoViewerIndex,
            onDismiss: { showingNotePhotoViewer = false },
            projectId: project.id
        )
        .environmentObject(dataController)
    }
    ...
    private var locationSection: some View {
        SectionCard(
            ...
            ZStack(alignment: .bottomTrailing) {
                let displayCoord: CLLocationCoordinate2D? = isEditingAddress
                    ? addressMapRegion.center
                    : project.coordinate
    ...
    /// Thumbnail view for gallery items (supports both project photos and note attachments)
    private func galleryThumbnailView(displayPhoto: DisplayPhoto, index: Int, allPhotos: [DisplayPhoto]) -> some View {
        ZStack(alignment: .bottomTrailing) {
            PhotoThumbnail(url: displayPhoto.url, project: project)
                .frame(width: 110, height: 110)
                .cornerRadius(OPSStyle.Layout.cardCornerRadius)
                .contentShape(Rectangle())
                .onTapGesture {
                    // Open full screen viewer with all display photos
                    notePhotoViewerURLs = allPhotos.map { $0.url }
                    notePhotoViewerIndex = index
                    showingNotePhotoViewer = true
                }
                .hoverEffect(.lift)
            ...
            // Comment count badge
            let commentCount = notesViewModel.commentCount(forPhotoURL: displayPhoto.url)
            if commentCount > 0 {
                HStack(spacing: 2) {
    ...
    // MARK: - Gallery with Note Photos

    /// All photos from both project images and note attachments, for combined gallery display
    private var allDisplayPhotos: [DisplayPhoto] {
        var result: [DisplayPhoto] = []

        // Project images (no caption)
        for url in project.getProjectImages() {
            result.append(DisplayPhoto(url: url, caption: nil, authorName: nil))
        }

        // Note attachment images (with caption from note content)
        for note in notesViewModel.notes {
            guard !note.attachments.isEmpty else { continue }
            let caption = note.content.prefix(80)
            let author = notesViewModel.authorName(for: note.authorId)
            for url in note.attachments {
                result.append(DisplayPhoto(
                    url: url,
                    caption: caption.isEmpty ? nil : String(caption),
                    authorName: author
                ))
            }
        }
...
struct ProjectDetailsView_OLD_Previews: PreviewProvider {
    static var previews: some View {
        ...
        sampleProject.endDate = Date().addingTimeInterval(60*60*24*30)
        sampleProject.notes = "This is a sample project for preview purposes."

        return ProjectDetailsView_OLD(project: sampleProject)
            .environmentObject(DataController())
    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                ...
                // Bottom bar: Annotate button (only if projectId is available)
...
/// Full-screen photo viewer with optional caption overlay for note attachments
struct CaptionPhotoViewer: View {
    let photos: [String]
    let captions: [String?]
    let authors: [String?]
    let initialIndex: Int
    ...
    init(photos: [String], captions: [String?], authors: [String?], initialIndex: Int, onDismiss: @escaping () -> Void, projectId: String? = nil) {
        self.photos = photos
    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                ...
                // Caption bar (for note photos)
                if currentIndex < captions.count, let caption = captions[currentIndex] {
                    HStack {
                        VStack(alignment: .leading, spacing: 2) {
                            if let author = authors[currentIndex] {
...
// MARK: - Inline Note Row (compact version for ProjectDetailsView)

struct InlineNoteRow: View {
    ...
    var onPhotoTap: (([String], Int) -> Void)? = nil
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: OPSStyle.Layout.spacing1) {
            ...
            if !note.attachments.isEmpty {
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack(spacing: OPSStyle.Layout.spacing2) {
                        ForEach(Array(note.attachments.enumerated()), id: \.element) { index, url in
                            PhotoThumbnail(url: url, project: nil)
                                .frame(width: 64, height: 64)
                                .clipShape(RoundedRectangle(cornerRadius: OPSStyle.Layout.cardCornerRadius))
                                .contentShape(Rectangle())
                                .onTapGesture {
                                    onPhotoTap?(note.attachments, index)
                                }
                        }
                    }
                }
