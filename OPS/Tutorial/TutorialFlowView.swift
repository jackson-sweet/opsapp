...
struct TutorialFlowView: View {
        ...
        return ZStack(alignment: .bottomLeading) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                Text(card.task.uppercased())
                    ...
                    .lineLimit(2)
                HStack(spacing: 6) {
                    Image(OPSStyle.Icons.project).font(.system(size: 11))
                    Text(card.project.uppercased()).font(.caption)
                }.foregroundStyle(.white.opacity(0.7))
                HStack(spacing: 6) {
                    Image(OPSStyle.Icons.teamMember).font(.system(size: 11))
    ...
    private func handleLeadTap() {
        ...
        withAnimation(OPSStyle.Animation.fast) { headerOpacity = 0 }
    ...
    private func handleReviewSwipe(direction: String) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.4) {
            ...
            withAnimation(OPSStyle.Animation.fast) { completionOpacity = 1 }
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.4 + holdTime) {
            withAnimation(OPSStyle.Animation.fast) { completionOpacity = 0 }
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.4) {
    ...
    private func showAllCaughtUp() {
        withAnimation(OPSStyle.Animation.fast) { showReviewStack = false }
    ...
    private func transitionToAccounting() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + t) {
            withAnimation(OPSStyle.Animation.fast) { showAccounting = true }
        }
        t += 0.4
        ...
        for i in 0..<swipeResults.count {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + cardTime) {
                withAnimation(OPSStyle.Animation.fast) { visibleInvoices = i + 1 }
                TutorialHaptics.arrival()
            }
        }
        ...
        // Revenue bar fills
        DispatchQueue.main.asyncAfter(deadline: .now() + t) {
            withAnimation(OPSStyle.Animation.fast) { invoiceBarProgress = 1 }
        }
        t += 0.8
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + t) {
            withAnimation(OPSStyle.Animation.fast) { showExpenses = true }
        }
        t += 0.3
        ...
        for i in 0..<allExpenses.count {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + expTime) {
                withAnimation(OPSStyle.Animation.fast) { visibleExpenses = i + 1 }
            }
        }
        ...
        // Expense bar fills
        DispatchQueue.main.asyncAfter(deadline: .now() + t) {
            withAnimation(OPSStyle.Animation.fast) { expenseBarProgress = 1 }
        }
        t += 0.7
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + t) {
            withAnimation(OPSStyle.Animation.fast) { showProfit = true }
        }
        t += 0.3

        DispatchQueue.main.asyncAfter(deadline: .now() + t) {
            withAnimation(OPSStyle.Animation.fast) { profitBarProgress = 1 }
            TutorialHaptics.milestone()
        }
        t += 1.5
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + t) {
            withAnimation(OPSStyle.Animation.smooth) { showAccounting = false }
        }
        t += 0.5
    ...
    private func startClosingSequence() {
        ...
        for i in 0..<opsStepCount {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + strikeTime) {
                withAnimation(OPSStyle.Animation.faster) { closingStrikeCount = i + 1 }
            }
        }
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + t) {
            withAnimation(OPSStyle.Animation.fast) { closingCollapsed = true }
            TutorialHaptics.milestone()
        }
        t += 0.6
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + t) {
            withAnimation(OPSStyle.Animation.fast) { showClosingMessage = true }
        }
        t += 0.8
