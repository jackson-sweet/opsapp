...
- **Fresh simulator clones ship with accessibility bridging OFF**, and SwiftUI then publishes no accessibility nodes in-process — AX-measuring suites (`TabBarHitTargetTests`) detect this via a control probe and skip with instructions instead of failing on the wrong cause. The long-lived `iPhone 17` sim already has it on. When creating a per-session clone, enable it once and reboot the device: `xcrun simctl spawn <udid> defaults write com.apple.Accessibility AccessibilityEnabled -bool true` plus the same for `ApplicationAccessibilityEnabled`.
- **The keyboard pixel-proof suite needs a host-side capture bridge.** Two tests in `SiteVisitTypeSettingsKeyboardTests` (`testVisitNameReceivesVisibleDoneInTheRealEditorSheet`, `testChecklistFieldLabelReceivesVisibleDoneInTheRealEditorSheet`) prove the software keyboard's DONE with the simulator's real screen pixels, which an app-hosted test cannot capture itself (`drawHierarchy` omits the remote keyboard; `XCUIScreen` is refused without UI-testing authority). From the ops-ios checkout, start `python3 scripts/testing/capture_keyboard_screens.py --udid <booted simulator udid> --max-seconds 600`, wait for its `"ready": true` line, then run `-only-testing:OPSTests/SiteVisitTypeSettingsKeyboardTests` against that same simulator with `-parallel-testing-enabled NO` (parallel testing runs the suite on a clone whose udid the bridge cannot know). Without a live bridge the two tests skip with that exact command; a running bridge that fails to answer is a real failure. The bridge exits after the suite's four captures, so start it again for every run. Late in a full `OPSTests` run an earlier test leaves the process unable to dismiss any keyboard; the suite's control probe then skips all three tests with the command to run the suite on its own — a skip there is the environment, not the editor.
...
- **Custom `ScrollTargetBehavior` eats `ScrollViewProxy.scrollTo` on iOS 26.** `updateTarget` is consulted for programmatic scrolls too, and receives the target view's own small rect (not a viewport-sized destination); rewriting its origin re-targets the scroll to an on-screen rect and the scroll silently no-ops (this broke tab-bar reveal-on-select). Guard: only rewrite targets whose `rect.width ≈ context.containerSize.width` (gesture-end targets) — see `PeekSnapBehavior` in `CustomTabBar.swift`.
- **Test-created UIWindows never execute programmatic SwiftUI scrolls (iOS 26.5).** In XCTest, `scrollTo` and `scrollPosition(id:)` both no-op in windows the test creates — detached or scene-attached — while direct `UIScrollView.setContentOffset` works. Assertions that a programmatic scroll happened must host the view in the app host's key window (swap `rootViewController`, restore after) — see `settledLaneState` in `OPSTests/Views/TabBarSnapshotTests.swift`.
- **SwiftData `rollback()` leaves already-held instances stale (iOS 26.5).** The store reverts correctly, but any `@Model` reference already in hand keeps serving its pre-rollback snapshot until an id-scoped fetch touches its row — the fetch rebinds the SAME instance (identity preserved, no trap). After rolling back a failed transaction, re-fetch every entity it mutated before callers re-read them (see `rematerializeAfterRollback` in `DataController.restoreTrash`). Related: an in-memory `allowsSave: false` ModelConfiguration no longer loads at all (its /dev/null-backed SQLite store fails to open read-only, NSCocoaErrorDomain 257) — build save-rejecting test fixtures by creating the store writable on disk, then reopening the same URL with `allowsSave: false` (see `makeSaveRejectingContainer` in `OPSTests/Views/TrashRecoveryTests.swift`).
- **A `ModelContext` does not keep its `ModelContainer` alive — `try makeContainer().mainContext` is a landmine.** The container deallocates at the end of the expression and the next `context.insert(…)` traps with an uncatchable `EXC_BREAKPOINT` inside SwiftData, before the test's first assertion. Every test dies in 0.000 s on a fresh process, which reads as "23 assertion failures" in the log and hides the real cause — the crash report's stack (`SwiftData → makeOp → insert`) is the only honest signal. Test helpers that vend a context must retain the container for the case's lifetime (see `retainedContainers` in `OPSTests/Sync/SyncCrossEntityDependencyTests.swift`).
- **`??` inside a `#Predicate` `contains(...)` operand aborts the process at fetch time (iOS 26.6).** SwiftData lowers `??` to a TERNARY. Core Data compiles that inside an ordered comparison — `(task.startDate ?? floor) >= start` runs on every device — but rejects it as the operand of `IN` (`unimplemented SQL generation for predicate … (bad LHS)`), and the rejection is an Objective-C exception inside `performAndWait` that Swift cannot catch: the process aborts, silently when the fetch runs off-main. Bug 7a726160 — `deckIds.contains($0.deckDesignId ?? "")` in `RecoveryAttentionReader` killed the app 3–6 s after every home-swipe (a backgrounded upload failing was enough to reach the branch; eleven identical crash reports on one phone in one day, all `SIGABRT` from an `NSManagedObjectContext` queue thread). Test the optional against `nil` inside the predicate and coalesce in Swift after the fetch. `OPSTests/Sync/SwiftDataPredicateLintTests.swift` scans the app target for the shape. To read crash reports off a paired iPhone: `xcrun devicectl device info files --device <udid> --domain-type systemCrashLogs`, then `copy from` — the `.ips` symbolicates OPS frames against the matching `OPS.debug.dylib` UUID in DerivedData via `atos`.
- **`contains(...)` over an array-backed `@Model` attribute inside a `#Predicate` SEGFAULTS the process (iOS 26.5).** Different failure from the `??` case above and worse: Core Data compiles the array search into `_NSCoreDataStringSearch`, which dereferences a null `CFString` — `EXC_BAD_ACCESS` / `SIGSEGV` at `0x0`, nothing to catch. Bug (2026-09-09) — `$0.assigneeIds.contains(user)` in `SiteVisitCaptureViewModel.openVisits()`. It hid for months because SQLite short-circuits `OR`: `createdBy == user` matched first for visits the operator made themselves, so the array term never ran; a visit a colleague created and assigned onward reaches it and kills the app mid-console. Measured, not assumed — the array term crashed in every combination probed: empty needle over an empty or populated array, a real needle over an empty array, a real needle over a MATCHING array, and the term alone with no `OR` around it. **A `contains` over a stored `String` attribute is fine** and stays legal (`MainTabView`'s field-role project fetch searches `teamMemberIdsString` on every launch — proved safe over both empty and populated rows). Fetch on the scalar terms and filter array membership in Swift afterwards. `SwiftDataPredicateLintTests.testNoContainsOverAnArrayBackedAttributeInAnyPredicate` reads the array-typed property names out of the `@Model` definitions and scans the app target, so String searches are not swept in. Note the crash-report tell: a missing `@EnvironmentObject` traps as `EXC_BREAKPOINT`/`SIGTRAP` WITH a full faulting stack, whereas this one reports as "Test crashed with signal segv" and can leave no `.ips` at all — when a snapshot test dies on its second render but its first render attached a valid PNG, suspect the store the first render wrote, not the canvas size.
- **A `#Predicate` fetch of `SyncOperation` TRAPS against a table that has never held a row.** Uncatchable `EXC_BREAKPOINT` inside SwiftData — not a thrown error — and it reproduces in every fresh in-memory container, so any test whose code path reaches `acceptableFields` / `hasPendingOperations` (the inbound conflict guards) dies on the fetch. Remedy: seed one inert warm-up `SyncOperation` into the test container before exercising those paths (see `makeContainer` in `OPSTests/Sync/CatalogMergeDiffGateTests.swift`); production code that must stay crash-proof fetches predicate-free and filters in Swift instead — see the note on `ProjectCacheMerge.operations` (`OPS/Network/Sync/ProjectCacheMerge.swift:63`).
- **`DataController` never probes auth in an XCTest process — keep it that way.** Its init spawns a one-shot `checkExistingAuth()` whose no-credentials path ends in `clearAuthentication()` seconds later: a keychain wipe plus dozens of UserDefaults removals that used to land mid-test, nilling the fixture-seeded `currentUser` (every guarded read then returns `[]`, which presents as a data bug far from the cause). The app host's OWN controller sprayed the same teardown into whichever test was running, so no test-side bracket can be airtight — init detects XCTest (`NSClassFromString("XCTestCase")`) and skips the probe instead. Fixtures construct `DataController()` bare and seed `currentUser` directly. Never re-add a settle loop that races the teardown (unwinnable — caused the 31b7d523 false red), and never weaken the init guard; UI tests run the app in a separate XCTest-free process and still probe normally.
- **Resolve the app host's window via `AppHostWindow.acquire()` — never naive `isKeyWindow` / `connectedScenes.first` lookups (iOS 26.5).** Hiding a UIWindow neither detaches it from its scene nor hands key status back deterministically, so full-suite runs accumulate hidden leftover harness windows in `scene.windows`; the host can also drop out of the foreground-active pipeline mid-suite, at which point window-level `drawHierarchy` of any freshly attached window renders blank suite-wide ("[Snapshotting] … requires it to be in a foreground scene") and later-transaction programmatic scrolls are dropped outright while initial-transaction scrolls and `setContentOffset` keep working. `AppHostWindow.acquire()` (`OPSTests/Views/AppHostWindow.swift`) picks the app's real window and repairs scene activation, visibility, and key status. Snapshot by drawing the hosted VIEW inside the app's window — never by drawing a test-created window (only pixel-asserting tests notice the blank; proof-PNG tests pass silently) — and settle pre-assertion state on geometry quiescence, never fixed sleeps.
