...
## UI POLISH
- [x] Filter sheet: swap Done/Reset buttons (Done → right side)
  - **Fixed**: Swapped button placements in `FilterSheet.swift`

---

## ANALYTICS
- [x] Track onboarding page views & compute completion progress per download
  - **Implemented**: Created `OnboardingAnalyticsService.swift` to send events to Bubble
  - **Requires Bubble setup**: Create `OnboardingEvent` data type and `track_onboarding_event` workflow

---

## CONTENT
- [x] Update "Help me choose" plan descriptions
  - **Fixed**: Clarified that all plans have full feature access, only seat limits differ
...
- **3 polish/analytics/content items completed**
