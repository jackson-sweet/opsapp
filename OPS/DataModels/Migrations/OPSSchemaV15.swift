...
//  OPS
//
//  Schema version 15.0.0 — site-visit → timeline auto-post idempotency.
//
//  V15 widens the local-only `SiteVisit` model with a single nullable field,
...
//
//  `SiteVisit` shipped through V11–V14 inside `v11SiteVisitModel`, so widening
//  it in place would shift EVERY V11–V14 schema's fingerprint by the same delta
//  — the hazard documented in `OPSApp.swift`. Following the V10→V11 (optional
//  opportunityId) and V13→V14 (Activity parents) precedents exactly, the
//  pre-widening shape is frozen as `OPSSchemaLegacySiteVisitV11.SiteVisit` and
//  backs V11–V14 (`v11SiteVisitModel`); the live widened `SiteVisit` backs V15+
//  (`v14SiteVisitModel`). Because the two shapes differ by a real persistent
//  attribute, V15's checksum diverges from V14 organically.
//
//  Purely additive at the SQLite level — one new nullable column. No rename,
//  retype, or drop. SwiftData lightweight migration handles the on-disk V14
...
enum OPSSchemaV15: VersionedSchema {
