...
public enum SurfaceReconciler {
    ...
    public static let rebindThreshold: Double = 0.5
    ...
    public static func migratedFromLegacy(
        detected: [DetectedSurface],
...
public struct DeckResolvedSurfacePayload: Equatable {
    public let persistedId: String?
    public let assignedItems: [AssignedItem]
    public let label: String?
    public let color: String
    public let boardMaterial: String
}

public struct DeckSurfaceSelectionSummary: Equatable {
    public let surfaceCount: Int
    public let title: String
    public let areaSquareInches: Double
    public let perimeterInches: Double
    public let assignedItems: [AssignedItem]
    public let label: String?
    public let color: String?
    public let boardMaterial: String?

    public var areaSquareFeet: Double { areaSquareInches / 144.0 }
}

public enum DeckSurfaceInspector {
    public static func primarySurfaceId(among surfaces: [DetectedSurface]) -> String? {
        surfaces.max(by: {
            abs(PolygonMath.signedArea(vertices: $0.positions)) < abs(PolygonMath.signedArea(vertices: $1.positions))
        })?.id
    }

    public static func resolvedPayload(
        detected: DetectedSurface,
        persisted: [DeckSurface],
        legacyFootprint: DeckFootprint,
...
public enum SurfaceDetector {
    ...
    public static func detect(vertices: [DeckVertex], edges: [DeckEdge]) -> [DetectedSurface] {
