...
- `OPSTests/SiteVisitChoiceResolutionTests`: seven real receipt-application regressions. Explicit pending resolution of an answer collision adopts the exact reviewed canonical ID and snapshot; original v1 same-ID recovery may hydrate the reviewed snapshot only when the original command omitted it. Both historical option generations retain the selected label Cedar. Incorrect snapshots or unreviewed IDs reject without changing local answers or queued request bytes. Same-ID v2 resolutions require the requested and reviewed snapshots to agree, and ordinary v2 saves retain their original snapshot check.
