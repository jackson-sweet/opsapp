...
//  OPSTests
//
//  Proves the V12→V13 staged migration is safe for the `Activity` widening —
//  `opportunityId` required→optional plus the new nullable `clientId`/`projectId`
//  (unified-activity parents). A real shipped store sits at V12 with a NOT NULL
...
//  the same file with the full migration plan and the current (V13) schema and
...
//  that version-scoping `Activity` (frozen legacy model + widened V13 model)
...
final class ActivityMigrationTests: XCTestCase {
    ...
    func test_v12StoreMigratesToV13_preservingOpportunityIdAndAllowingClientAndJobParents() throws {
        // 1. Stand up a V12 store with the frozen (required-opportunityId,
        ...
        try autoreleasepool {
            ...
            let v12Container = try ModelContainer(for: v12Schema, configurations: v12Config)
        ...
        //    This drives V12 → V13 (opportunityId becomes optional; clientId /
        ...
        XCTAssertEqual(activities.count, 1, "The V12 activity row must survive migration.")
        let migratedActivity = try XCTUnwrap(activities.first)
        XCTAssertEqual(migratedActivity.id, "act-v12")
        ...
        // 4. The migrated (V13) store can now persist a CLIENT-parented activity
