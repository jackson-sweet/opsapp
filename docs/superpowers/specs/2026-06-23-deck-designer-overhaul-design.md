...
| 5 | ~~3D measure tools~~ | **CANCELLED 2026-06-24 (Jackson):** no measure on the 3D view. Measure stays 2D-only (already exists in `DeckTab2DView`). | — |
...
- **Drop 5 — ~~3D measure tools~~ — CANCELLED (2026-06-24, Jackson).** No measure on the 3D view. Measure stays **2D-only**, which already exists in `DeckTab2DView` (ruler + select-and-measure breakdown). Nothing to build.
