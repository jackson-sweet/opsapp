...
class OnboardingViewModel: ObservableObject {
    ...
    func moveToNextStep() {
        ...
        // Use our custom next step navigation
        let nextStep = currentStep.nextStep()
        if let step = nextStep {
            print("OnboardingViewModel: Found next step: \(step.title) (raw: \(step.rawValue))")
            
            // Force onto main thread and add additional debug
    ...
    func moveToPreviousStep() {
        ...
        let prevStep = OnboardingStep(rawValue: currentStep.rawValue - 1)
        if let step = prevStep {
            print("OnboardingViewModel: Found previous step: \(step.title) (raw: \(step.rawValue))")
    ...
    // Track current step in V2 flow
    @Published var currentStepV2: OnboardingStepV2 = .welcome
    
    // Move to the next step in the V2 flow
    ...
    // Move to a specific step in V2 flow
